...
module.exports.run = async (bot, msg, args, pog) => {
  ...
  server.findOne({ Server: msg.guild.id }, async(err, data) => {
    ...
    if(data){
      ...
      msg.channel.send(`Successfully set server language to **${args[0].toLowerCase()} [${ISO6391.getName(args[0].toLowerCase())}]**\n[s!setlanguage en] To get back to default, and without any arguments for great info!`)
    } else {
      new server({
        Server: msg.guild.id,
        Language: args[0],
